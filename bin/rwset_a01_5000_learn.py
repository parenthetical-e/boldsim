...
from boldsim.base import RWset


def _run(name, model_conf, TR, ISI, alpha):
    """ Runs a single simulation, returning a simfMRI results object. """
    
    n = 60 ## Number of trials
    exp = RWfit(n, 'learn', TR, ISI, alpha)
...
if __name__ == "__main__":
    ...
    pmain = functools.partial(main, model_conf=model_conf, 
            TR=TR, ISI=ISI, alpha=alpha)
    results_in_chunks = pool.map(pmain, run_chunks)
    results = reduce_chunks(results_in_chunks)
    
    print("Writing results to disk.")
    results_name = "rwset_alpha{1}_{0}_learn".format(nrun, alpha)
